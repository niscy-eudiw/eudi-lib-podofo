#include "ExtractIssuerCertFromTSR.h"
...
std::string extractIssuerCertFromTSR(const std::string& base64Tsr) {
    Logger::info("=== Starting Issuer Certificate Extraction from TSR Service ===");
    ...
    if (!certs || sk_X509_num(certs) < 2) {
        Logger::error("TSR does not contain enough certificates to find issuer");
        throw std::runtime_error("TSR does not contain enough certificates to find issuer.");
    }

    Logger::info("Getting issuer certificate (index 1)...");
    X509* issuerCert = sk_X509_value(certs, 1);
    if (!issuerCert) {
        Logger::error("Could not get issuer certificate from TSR");
        throw std::runtime_error("Could not get issuer certificate from TSR.");
    }

    Logger::info("Encoding issuer certificate to DER...");
    int len = i2d_X509(issuerCert, nullptr);
    if (len <= 0) {
        Logger::error("Failed to get length of DER for issuer cert");
        throw std::runtime_error("Failed to get length of DER for issuer cert.");
    }
    std::vector<unsigned char> issuer_der(len);
    unsigned char* out_p = issuer_der.data();
    if (i2d_X509(issuerCert, &out_p) <= 0) {
        Logger::error("Failed to encode issuer cert to DER");
        throw std::runtime_error("Failed to encode issuer cert to DER.");
    }

    Logger::info("Encoding issuer certificate to base64...");
    std::string result = base64_encode(issuer_der);
    Logger::debug("Base64 encoded issuer certificate size", std::to_string(result.length()) + " characters");
    Logger::info("Issuer certificate extraction completed successfully");
    Logger::info("============================================================");
    return result;
}

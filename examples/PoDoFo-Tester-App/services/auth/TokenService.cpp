...
std::string TokenService(const std::string& code) {
    ...
    Logger::info("=== Starting Service Token Service ===");
    ...
    try {
        ...
        const std::string url = Config::getOAuthTokenUrl();
        ...
        if (res != CURLE_OK) {
            throw std::runtime_error("Service token request failed: " + std::string(curl_easy_strerror(res)));
        }
        ...
        Logger::error("Exception in TokenService: " + std::string(e.what()));
        Logger::debug("Full response (if any)", response);
        if (curl) curl_easy_cleanup(curl);
    }

    Logger::info("Service token service completed");
    Logger::info("=============================");
    return accessToken;
}
